 ...
 * 注意事项:
 */
class MyCollectedBlogsViewModel : BaseViewModel() {
    private val repository by lazy { BlogRepository() }
    val liveData_complete = MutableLiveData<Any>()
    fun query_collects(page: PageBean): MutableLiveData<CommonResultBean<BlogPageListResult>> {
        val resultLiveData = MutableLiveData<CommonResultBean<BlogPageListResult>>()
        launch(
            workBlock = {
                var result = repository.query_collects(page)

 ...
 * 注意事项:
 */
class MyAddedBlogsViewModel : BaseViewModel() {
    private val repository by lazy { BlogRepository() }
    val liveData_complete = MutableLiveData<Any>()
    fun query_my_added_blogs(page: PageBean): MutableLiveData<CommonResultBean<BlogPageListResult>> {
        val resultLiveData = MutableLiveData<CommonResultBean<BlogPageListResult>>()
        launch(
            workBlock = {
                var result = repository.query_my_added_blogs(page)
